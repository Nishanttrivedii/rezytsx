    ...
    <div className={isLargeScreen ? 'm-4' : ''}>
      ...
      <ul>
        {Devices.map((device: Device) => (
          <li
            key={device.issue}
            className={`w-[100%] h-[5rem] bg-[${device.bg}] mt-2 items-center flex rounded border-1 border-[#FF9900] flex-col gap-1`}
            style={{ backgroundColor: device.bg, borderWidth: '1px', borderColor: '#FF9900' }}
          >
            <div className='w-[100%] h-[50%] flex mt-2 justify-between'>
              ...
              <div className='order-2 mr-2 flex flex-row gap-1' style={{ color: device.text }}>
                <div>
                  Building {device.building} - Unit {device.unit}
                </div>
